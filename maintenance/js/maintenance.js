	...
	if(isExists('#normal-countdown')){
		...
		$('#normal-countdown').countdown(date, function(event) {
			var $this = $(this).html(event.strftime(''
				+ '<div class="time-sec"><h3 class="main-time">%D</h3> <span>Días</span></div>'
				...
				+ '<div class="time-sec"><h3 class="main-time">%S</h3> <span>Segs</span></div>'));
		});
	}
...
function countdownTime(){
	...
	if(isExists('#clock')){
		$('#clock').countdown('2020/03/25', function(event){
			var $this = $(this).html(event.strftime(''
...
function initMap() {
	...
	var uluru = {lat: 38.6944556, lng: -62.2485326};
